use crate::{TypedGraph, SchemaExt, Key, Id, SchemaResult, Typed};
    ...
    Progress: Iterator<Item = (State, SchemaResult<T, NK, EK, S>)> + 'a
{
    ...
    pub fn set_state<NewState>(
        self, 
        new_state: NewState
    ) -> GraphWalker<'a, T, NewState, NK, EK, S, impl Iterator<Item = (NewState, SchemaResult<T, NK, EK, S>)>> 
    where
        NewState: Clone
    {
        GraphWalker {
            g: self.g,
            front: self.front
                .map(move |(_, res)| (new_state.clone(), res))
        }
    }
    ...
    ) -> GraphWalker<'b, NewT, State, NK, EK, S, impl Iterator<Item = (State, SchemaResult<NewT, NK, EK, S>)> + 'b> 
    where
        'a: 'b,
        'b: 'a,
        NewT: 'b,
    ...
    pub fn one_with_state(mut self) -> SchemaResult<Option<WalkerTarget<T, State>>, NK, EK, S> {
        self.front
            ...
            .map_or_else(
                ...
                res.map_or_else(
                    |e| Err(e), 
                    |t| Ok(Some(WalkerTarget { val: t, state }))
                )
            )
    }
