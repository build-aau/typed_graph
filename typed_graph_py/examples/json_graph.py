...
class Weight(RootModel, NodeExt[int, str], EdgeExt[int, str]):   
    """
...
class JsonSchema(SchemaExt[Weight, Weight, int, int, str, str]):
    version: str
    nodes: List[str]
...
JsonGraph = TypedGraph[Weight, Weight, int, int, str, str, JsonSchema]
...
if __name__ == '__main__':
    ...
    g = JsonGraph(s)
    # Populate the graph
    a_id = g.add_node(Weight(**{'id': 0, 'type': 'A'}))
    b_id = g.add_node(Weight(**{'id': 1, 'type': 'B'}))
    c_id = g.add_node(Weight(**{'id': 2, 'type': 'C'}))
    ab_id = g.add_edge(a_id, b_id, Weight(**{'id': 0, 'type': 'AB'}))
    bc_id = g.add_edge(b_id, c_id, Weight(**{'id': 1, 'type': 'BC'}))
    ca_id = g.add_edge(c_id, a_id, Weight(**{'id': 2, 'type': 'CA'}))
    cc_id = g.add_edge(c_id, c_id, Weight(**{'id': 3, 'type': 'CC'}))


    # trying to add a type that is not part of the schema will result in  an exception
    try:
        g.add_node(Weight(**{'id': 4, 'type': 'D'}))
    except Exception as e:
        print(e)

    # The same thing happens when trying to add an edge with a type that is not allowed
    try:
        g.add_edge(c_id, a_id, Weight(**{'id': 4, 'type': 'DD'}))
    except Exception as e:
        print(e)
    ...
    dublicate_edge_id = g.add_edge(a_id, b_id, Weight(**{"id": 3, "type": "AB"}))
    g.remove_edge(dublicate_edge_id)

    # Loops are also allowed as long as the are part of the schema
    dublicate_edge_id = g.add_edge(c_id, c_id, Weight(**{"id": 3, "type": "CC"}))
    g.remove_edge(dublicate_edge_id)
    ...
    g.add_node(a)
    g.add_edge(a_id, b_id, Weight(**{"id": 0, "type": "AB"}))
    g.add_edge(c_id, a_id, Weight(**{"id": 2, "type": "CA"}))
